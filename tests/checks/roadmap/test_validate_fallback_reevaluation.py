...
Cases that don't need real git (CD.27-absent, malformed roadmap) use the plans_dir/roadmap_path
injection seams directly.
"""
...
class TestNoGitNeeded:
    ...
    def test_cd27_absent_fails_loud(self, tmp_path: Path, capsys) -> None:
        ...
        failed: list[str] = []
        validate_fallback_reevaluation(failed, plans_dir=tmp_path / "plans", roadmap_path=roadmap_path)
    ...
    def test_missing_roadmap_file_fails_loud(self, tmp_path: Path, capsys) -> None:
        failed: list[str] = []
        validate_fallback_reevaluation(failed, plans_dir=tmp_path / "plans", roadmap_path=tmp_path / "does-not-exist.yaml")
        assert failed == ["Fallback re-evaluation carrier"]
...
class TestGitBackedFiring:
    ...
    def test_not_net_new_plan_is_untouched(self, tmp_path: Path) -> None:
        ...
        _init_repo(repo)
        _write_roadmap(repo)
        _write_plan(repo, "fr-not-net-new", "Platform T4.2 atomic-plan work.", closes_criteria=["T4.2:c1"])
