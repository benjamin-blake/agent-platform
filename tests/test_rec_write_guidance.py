...
class TestLoadSourceRegistry:
    ...
    def test_returns_33_entries(self) -> None:
        """load_source_registry() returns 33 canonical entries (includes ci_rca_evidence_dispute)."""
...
class TestGetRecWriteGuidance:
    ...
    def test_registered_values_has_33_entries(self) -> None:
        """'registered_values' list has 33 registry entries (includes ci_rca_evidence_dispute)."""
        from scripts.executor.rec_write_guidance import get_rec_write_guidance

        guidance = get_rec_write_guidance()
        assert len(guidance["source"]["registered_values"]) == 33
