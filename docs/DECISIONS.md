...
2. **Forward routing rule (the "stop the bleeding" clause).** No new `docs/INTENT-*.md` files. Architectural content routes to its canonical machine-parseable home by type:
...
3. **The existing 18 are grandfathered** and retired wave-by-wave per `docs/intent-migration/MANIFEST.yaml` (Waves 1-5), each with its own per-doc drift reconciliation. A `scripts/validate.py` guard (added in Wave 0) rejects NEW `docs/INTENT-*.md` while allowing the shrinking grandfather set.
...
## Decision 83: Branch Protection Now Active -- Amends Decision 72 Premise (Decided)
...
**Decision:**
Per the evidence in `docs/INTENT-bazel-feasibility.md` (a C1-C10 claims-verification matrix at commit `ddb85a0`, merged in PR #64):
...
**Related:** Decision 43 (validate.py SLOC waiver this remediates), Decision 60 / Decision 73 (two-tier diff-aware CI the do-less path extends), Decision 75 (Frame-Lock Anti-Pattern, applied to the assessment itself), Decision 67 / Decision 79 (STRATEGIC freeze retained; Lambda-deploy lifted); ROADMAP KG.13, T4.1/T4.4, CD.27 (revisit triggers); `docs/INTENT-bazel-feasibility.md` (supporting analysis, PR #64).
...
- Decision 72 (Branch Protection Unavailable): the forward-fix model is designed around branch protection being permanently unavailable. *(Premise reversed by Decision 83, 2026-06-08; design consequences preserved.)*
