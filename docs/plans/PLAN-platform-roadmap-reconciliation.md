...
## Acceptance Criteria
- [ ] The six clear-cut migration-realized items (**T0.2, T0.3, T2.1, T2.3, T2.10, T2.13**) are `status: complete`, each with a `completed_at` confirmed against git history for the realizing artefact (not transcribed from this plan's assumed dates - see Step 3), `bootstrap_completion_exempt: true`, and a `note` citing the evidence (PR / artefact path). T2.1's note records its exit-criteria divergence (CD.21-retired runner; outstanding T0.4 inference creds).
...
3. **Flip the six clear-cut items** to `status: complete` with `bootstrap_completion_exempt: true` and an evidence `note`. **The PR numbers and dates below are starting hypotheses, not authoritative - confirm each `completed_at` against git history for the realizing artefact** (`git log --date=short --diff-filter=A -- <artefact>` for first-landing, or the merge date of the PR that actually realized the item) before writing it. Do not transcribe the assumed dates blindly.
   - T0.2 -> complete. Realizing substrate is `bin/setup-cloud-env.sh` (which landed via the static-creds PRs #4/#6, NOT the workflow-migration PR #10); cite Decision 76 for the CC-web dev surface. Confirm completed_at from the `setup-cloud-env.sh` first-landing commit (~2026-05-29/30).
   - T0.3 -> complete (`terraform/personal/platform_roles.tf`, static-key creds PR #4/#5; cite CD.26). Confirm completed_at (~2026-05-29). Note: T0.3 carries `user_action_required: true` and a deferred exit criterion - record in the note that the realized state is the agent-auth substrate, with any human-action exit criterion still outstanding.
   - T2.1 -> complete (personal-account Terraform Phase B applied+verified, PR #1; cite Decision 77). T2.1 is `strategic: true` with deps T0.4/T0.6 still `not_started`; its `note` MUST record the exit-criteria divergence per the Constraint on out-of-order completes: exit criterion "EC2 runner registration" is superseded (CD.21 retired the runner) and "inference creds" is outstanding (T0.4 not_started). Confirm completed_at (~2026-05-28).
   - T2.3 -> complete (`agent-platform` rename PR #1 + legacy-reference sweep PR #11; cite Decision 24). Confirm completed_at (~2026-05-30).
   - T2.10 -> complete (`terraform/personal/oidc.tf`, EC2 runner retired per CD.21; cite Decision 73). Confirm completed_at (~2026-05-28).
   - T2.13 -> complete + a note that the public flip landed ahead of CD.20's security/portal sequence (T2.11a/b + T2.12 outstanding). **Do NOT inherit PR #1 / 2026-05-28** - PR #1 was the rename/Phase B, not the visibility flip. Decision 76 (2026-05-30) already treats the repo as public, so confirm the actual visibility-flip PR/date (~2026-05-30 window) from git history before writing completed_at.
