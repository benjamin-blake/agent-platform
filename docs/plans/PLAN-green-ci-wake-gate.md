...
- [ ] On a red check, no wake comment is posted (`signal-green` skipped); the failure-path wake is left to the existing `subscribe_pr_activity` subscription (unchanged). The elimination of the old fast-tier comment -- which fired on `pr-validate` success even while `terraform-validate` was still running (or about to fail) -- is the intended fix, NOT a regression of the red-path wake (the subscription still wakes on the failed check event).
