...
| `docs/GETTING_STARTED.md` | Modify | Collapse the retired dual "Company Environment Setup" + "Personal Environment Setup" / SSO premise into the single current model. Sweep ALL retired tokens (not just headline ones): `aws sso login`, `your-aws-profile`->`agent_platform`, `Activate.ps1`/`.venv/Scripts`->`bin/venv-python`, `netstat ... findstr`->`ss`/`grep`, `COMPANY_S3_*`, `formulas-*`->`agent-platform-data-lake`, "company VM". ~986-line file. |
...
| `scripts/session_postflight.py` | Modify | The proactive pre-drain interactive `aws sso login --profile agent_platform` attempt (~L667-670, in `run_auto()`) is dead under the static-key auto-refresh model (Dec 57/60 degraded-mode; `_SSO_PROFILE` already resolves to the auto-refreshing `agent_platform`). Remove the interactive-login subprocess; let `_check_sso` gate the drain (skip-and-continue if False, no error spam). Leave `_check_sso`/`_SSO_PROFILE` names (cosmetic rename deferred). |
| `tests/test_session_postflight.py` | Modify | Add/adjust a test asserting `run_auto()` spawns no `aws sso login` subprocess, and that a False `_check_sso` skips the drain (skip-and-continue) rather than emitting a blocking error. (Opportunistically addresses rec-657's missing `check_sso` mock if trivial.) |
...
## Acceptance Criteria
- [ ] Operational/ambient docs (`PROJECT_CONTEXT.md`, `GETTING_STARTED.md`, `ARCHITECTURE.md`, `ARCHITECTURE-WORKFLOW.md`, `README.md`) contain none of (as current instructions): `company-aws-profile`, `your-aws-profile`, `aws sso login`, `formulas-{discovery,staging,production}`, `COMPANY_S3_`, `.venv/Scripts`, `Activate.ps1`, `C:/Users`, `pyenv ... 3.11`.
...
- [ ] `scripts/session_postflight.py` invokes no `aws sso login`; in `run_auto()` a False `_check_sso` skips the drain (skip-and-continue, no blocking error), relying on the auto-refreshing static-key chain.
...
|---|-------|--------|---------|-------------------|--------|
| 1 | pre-merge | Unambiguous legacy tokens gone from operational docs | `! git grep -nE "company-aws-profile\|your-aws-profile\|aws sso login\|formulas-(discovery\|staging\|production)\|COMPANY_S3_\|\.venv/Scripts\|Activate\.ps1\|C:/Users\|pyenv[^\n]*3\.11" -- docs/PROJECT_CONTEXT.md docs/GETTING_STARTED.md docs/ARCHITECTURE.md docs/ARCHITECTURE-WORKFLOW.md README.md` | Exit 0 (no matches printed) | A match means a stale current-instruction token survives -> replace with current truth (collapse GETTING_STARTED dual-env/SSO sections) |
...
| 5 | pre-merge | Decision addenda present (all 6 distinct) | `test "$(grep -cE "2026-05 migration update" docs/DECISIONS.md)" -ge 6` | Exit 0 (>=6 dated pointers) | Fewer than 6, OR two markers under one decision while another is skipped -> confirm each of Dec 60/68/71/72(branch)/24/62 individually carries its own pointer |
...
| 9 | pre-merge | postflight: no interactive SSO login; drain gated by _check_sso | `! git grep -nE "\"sso\", *\"login\"\|aws sso login" -- scripts/session_postflight.py && bin/venv-python -m pytest tests/test_session_postflight.py -q` | Exit 0 + pass | `aws sso login` subprocess remains, or the test does not assert skip-and-continue when `_check_sso` is False |
...
- **Plan-critique correction (session_postflight):** the `run_auto()` block at ~L667-670 is a *proactive pre-drain interactive SSO login* (not a credential-failure fallback). Under the static-key model (`_SSO_PROFILE = agent_platform`, auto-refreshing) it is dead code. Remedy = remove the interactive login + skip-and-continue when `_check_sso` is False (Dec 57/60 degraded-mode), NOT emit an error.
...
2. **Operational/ambient docs (present-tense, replace with current truth).** Edit `docs/PROJECT_CONTEXT.md` (lines 5, 15, 37, 238 only -- NOT File Router rows 147-173), then `docs/GETTING_STARTED.md`, `docs/ARCHITECTURE.md`, `docs/ARCHITECTURE-WORKFLOW.md`, `README.md`. Substitutions: `company-aws-profile`->`agent_platform`; `aws sso login`->`aws sts get-caller-identity --profile agent_platform` (static-key); `formulas-{discovery,staging,production}`->`agent-platform-data-lake`; Windows/Git-Bash/PowerShell/`.venv/Scripts`/pyenv-3.11.9->Linux/bash/`bin/venv-python`/3.12. For `GETTING_STARTED.md` specifically, collapse the dual Company/Personal-environment + SSO setup sections into the single current model and sweep ALL retired tokens (`aws sso login`, `your-aws-profile`, `Activate.ps1`, `netstat...findstr`, `COMPANY_S3_*`), not just the headline ones.
...
6. **Code/config leaks.**
   ...
   b. `scripts/session_postflight.py`: in `run_auto()`, remove the proactive interactive `aws sso login --profile agent_platform` subprocess (~L667-670) -- dead under the static-key auto-refresh model. Let `_check_sso` gate the drain: if False, log an informational skip and continue (skip-and-continue, Dec 57/60 degraded-mode); do NOT emit a blocking error. Leave `_check_sso`/`_SSO_PROFILE` names (cosmetic rename deferred). Update `tests/test_session_postflight.py` to assert no `aws sso login` spawn AND that a False `_check_sso` skips the drain without erroring.
