...
| docs/PROJECT_CONTEXT.md | Modify | Fix ALL THREE stale `docs/ROADMAP-PRODUCT.md` product-context pointers (prose intro, disambiguation rule, quick-reference table) to point at `docs/ROADMAP-PRODUCT.yaml` (canonical). |
...
- [ ] `docs/PROJECT_CONTEXT.md` contains no remaining bare `docs/ROADMAP-PRODUCT.md` product-context pointer: the prose intro, the roadmap-disambiguation rule, and the quick-reference table all point at `docs/ROADMAP-PRODUCT.yaml`.
...
| 4 | pre-deploy | Confirm CD.31's reference to the KG.1 boundary is intact and unchanged (exactly the 2 existing occurrences, both inside CD.31) | `bin/venv-python -c "import sys; n=open('docs/ROADMAP-PLATFORM.yaml').read().count('KG.1 platform/product boundary'); sys.exit(0 if n==2 else 1)"` | Exits 0 (count == 2; CD.31 discipline_points references preserved exactly) | Non-zero exit -> CD.31 was inadvertently edited (count != 2); revert CD.31, it must stay unchanged |
...
| 7 | pre-deploy | Confirm PROJECT_CONTEXT has zero remaining bare `ROADMAP-PRODUCT.md` pointers (all three product-context references now point at the `.yaml`) | `bin/venv-python -c "import sys; t=open('docs/PROJECT_CONTEXT.md').read(); sys.exit(0 if (t.count('ROADMAP-PRODUCT.md')==0 and 'ROADMAP-PRODUCT.yaml' in t) else 1)"` | Exits 0 (no `ROADMAP-PRODUCT.md` remains; `.yaml` is referenced) | Exit 1 -> a stale `.md` pointer survives (check the prose intro, the disambiguation rule, and the quick-reference table) |
...
5. **`docs/PROJECT_CONTEXT.md` -- fix all three stale product-roadmap pointers.** Replace every `docs/ROADMAP-PRODUCT.md` product-context reference with `docs/ROADMAP-PRODUCT.yaml` (canonical): (a) the prose intro line "See docs/ROADMAP-PRODUCT.md for product phases ..." (~line 83); (b) the "Product context" bullet in the "Roadmap reference disambiguation" section (~line 89); (c) the quick-reference table row "Product roadmap (phases, milestones)" (~line 119). Do NOT add any new `.md`-naming note -- leave the file free of bare `ROADMAP-PRODUCT.md` pointers so VP step 7 can assert zero. Do not otherwise restructure the section. (The bare `docs/ROADMAP-PRODUCT.md` -> `.yaml` rename is the only change; the `.md` recovery artefact itself is left untouched on disk.)
