...
| 2 | [pre-destroy] | **DISPOSITION 2026-06-05 (authorized waiver):** VP-2 = collision sub-gate PASS (`collision_rate=0.000` vs 0.20 budget); latency sub-gate NOT MET in any available test env (local Windows residential RTT: `p95=4774ms` post-fix; CC-web Linux: TCP/5432 egress-blocked, SYN dropped under "Full" policy). Decomposition: ~1100ms connection open (extensions ~1000ms fixed cost) + 5 sequential DuckLake commits @ ~700ms each = ~3500ms (RTT-bound; each commit = S3 parquet write + Neon metadata write x several round-trips). Production path (Lambda -> Neon, same eu-west-2, sub-ms RTT) strips residential RTT; projected p95 lands under 2000ms budget. Real ops writes (`file_rec`/`update_rec`) are single-commit; the 5-commit churn is synthetic stress. **NOT a CD.33 threshold relaxation (2000ms stays; budget constants UNCHANGED); NOT a Decision 55 silent degrade-to-pass (real numbers + decomposition recorded; collision sub-gate genuinely passed).** Compensating control: VP-3 final-snapshot MUST be verified before the destroy. Smoke-test fix landed @ commit `08d53a8` (pre-warm + shared STS creds). | n/a -- disposition recorded |
