...
| scripts/verifiers/athena_views.py | Modify | Repoint the Athena connectivity liveness probe (:52) off `ops_recommendations_current` (dropped in slice 6) onto a surviving view (`ops_decisions_current`). Without this the verifier returns `VerifierStatus.FAIL` once the view is gone, regressing VP 15 / the verifier harness. |
...
- [ ] After the view drop, the `validate.py` verifier harness still passes (`scripts/verifiers/athena_views.py` repointed off the dropped view); no non-deferred source file references `ops_recommendations_current` (VP 16).
...
| 16 | [pre-deploy] | No stray dropped-view refs (self-checking sweep) | `test -z "$(grep -rn ops_recommendations_current scripts/ src/ --include='*.py' | grep -vE 'migrate_ops_data|cleanup_ops_rec_orphans')" && echo NO_STRAY_VIEW_REFS` | prints `NO_STRAY_VIEW_REFS` -- only the archival-deferred legacy tools reference the dropped view | a non-deferred source still references `ops_recommendations_current` (e.g. an unrepointed verifier) -> repoint it |
...
- **Remaining `ops_recommendations_current` references after this plan** are ONLY the archival-deferred legacy tools above (the VP-16 sweep guard enforces this). `scripts/verifiers/schema_integrity.py` reads the RETAINED Iceberg recs TABLE schema via the Glue catalog (not the `_current` view), so it survives the view drop and stays valid while the Iceberg rollback table is retained; repoint/retire it when the Iceberg recs table is itself dropped (future slice, out of scope here).
...
6. **Slice 6 -- drop the view + doc/contract sync (AFTER slice 2 lands).** `terraform/personal/main.tf`: drop the `ops_recommendations_current` view. CAUTION -- it is created by `null_resource.create_ops_views` (for_each, create-time provisioner only); removing its `create_ops_view_queries` entry destroys the null_resource in state but issues NO `DROP VIEW`, so the live Glue view would persist and VP 12 would fail. Add an explicit `DROP VIEW IF EXISTS agent_platform.ops_recommendations_current` (destroy-time `local-exec` provisioner on the view resource OR a one-shot Athena DDL step). This is a `null_resource` delete, so the apply-guard fail-closes -> MANUAL admin-apply (Decision 77). BEFORE/with the drop, repoint `scripts/verifiers/athena_views.py`'s liveness probe (:52) off `ops_recommendations_current` onto `ops_decisions_current` so the verifier harness (VP 15) stays green. Update `docs/contracts/ops-data-store.md`, `docs/contracts/read-engine.yaml`, `docs/ARCHITECTURE-WORKFLOW.md`. Do NOT touch `.github/copilot-instructions.md` (deep-frozen). Do NOT relabel `config/lambda/ducklake/field_semantics.yaml` `current_table` -- it is runtime-load-bearing (feeds the live DuckLake `MERGE INTO`/`SELECT FROM {current_table}` SQL in `ducklake_scd2_schema.py`/`ducklake_runtime.py`); it is in the DuckLake catalog namespace (no collision with the dropped Glue view) and must stay as-is.
