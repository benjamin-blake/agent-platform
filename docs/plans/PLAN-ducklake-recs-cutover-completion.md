...
**Lambda Deployment Assessment (Decision 79 / CD.16+CD.24).** `compute_affected_artifacts(changed_files)` returns five `status: active` artifacts; disposition per artifact:
- **ops-compaction** (bundles `src/data/handlers/ops_compaction_handler.py` + `scripts/ops_writer.py` + `src/common/iceberg_reader.py`, all edited; LIVE -- S3-triggered on the staging prefix, NOT disabled): **build + deploy + smoke-test REQUIRED** (VP 8 unit, VP 13 deploy, VP 14 live smoke). Its `OpsWriter.compact()` behaviour changes (recs excluded), so the deployed code must be refreshed.
- **data-pipeline** (bundles `src/data/handlers/scheduled_agent_handler.py` + `scripts/ops_writer.py` + `src/common/iceberg_reader.py`, all edited): **evidenced deploy-DEFERRAL.** The dispatcher's EventBridge trigger is `state = "DISABLED"` with `SCHEDULED_AGENTS_ENABLED = "false"` (`terraform/scheduled_agents.tf`), so the edited recs-read path is never invoked in production, and the AGENTS.md "Re-enable Lambda scheduled agents" runbook mandates a fresh `build_lambda --deploy` at re-enable that will pick up these edits. The co-bundled `findings_processor` neither reads nor writes recs, so its behaviour is unchanged. Deferral recorded; no redeploy in this plan.
- **ducklake_reader / ducklake_writer / ducklake_maintenance**: edits limited to the manifests' `runtime_config[]` declarations -- build/SSOT metadata consumed by the client-side `make_reader` resolver, NOT read by the Lambda at runtime -- so NO zip rebuild/redeploy is required. The post-apply live smoke (VP 10-11) confirms the reader/writer Function URLs still serve under SSM discovery.
...
6. **Slice 6 -- drop the view + doc/contract sync (AFTER slice 2 lands).** `terraform/personal/main.tf`: remove the `ops_recommendations_current` view DDL. Update `docs/contracts/ops-data-store.md`, `docs/contracts/read-engine.yaml`, `docs/ARCHITECTURE-WORKFLOW.md`. Do NOT touch `.github/copilot-instructions.md` (deep-frozen). Do NOT relabel `config/lambda/ducklake/field_semantics.yaml` `current_table` -- it is runtime-load-bearing (feeds the live DuckLake `MERGE INTO`/`SELECT FROM {current_table}` SQL in `ducklake_scd2_schema.py`/`ducklake_runtime.py`); it is in the DuckLake catalog namespace (no collision with the dropped Glue view) and must stay as-is.
