...
| File | Action | Purpose |
|------|--------|---------|
| config/lambda/ducklake/field_semantics.yaml | Modify | Extend the SCD2 field-semantics contract from smoke-only to the real `ops_*` schemas (ops_recommendations, ops_decisions, ops_execution_plans, ops_session_log, ops_priority_queue): per-table column maps, types, input/derived roles, partition transforms. This is the single source of truth for both the write-time schema gate AND the DDL. |
| src/common/ducklake_runtime.py | Modify | Generalize `create_scd2_tables` / `write_scd2` / `read_current` / `schema_gate` / the MERGE SQL from smoke-table-hardcoded to **table-parameterized** (driven by field_semantics per table). Keep the smoke constants + signatures as back-compat defaults (T2.17 smoke gates must still pass). Mint-once-outside-OCC rule preserved (CD.33 clause 2). |
...
| scripts/ops_data_portal.py | Modify | Swap `file_rec` / `update_rec` / `file_decision` / `update_decision` transport from `OpsWriter().write()` (Iceberg) to invoking the `ducklake_writer` Function URL (SigV4). Preserve the portal surface verbatim (Decision 78 cl.6 / Decision 81 cl.4 -- Single-Portal invariant), DynamoDB ID allocation, and the offline outbox semantics. Gate the transport on `OPS_STORAGE_BACKEND` (rollback flag). Reads route via the DuckLake reader. |
...
| scripts/data_quality_runner.py | Modify | Read `ops_*` from the DuckLake reader instead of Athena for the ops-table checks (telemetry tables unchanged this slice). |
...
| tests/test_ops_data_portal.py | Modify | Transport-swap + flag tests: portal surface unchanged; `ducklake` backend invokes the writer; `iceberg` backend still works (rollback); ID allocation + outbox preserved. |
...
## Acceptance Criteria
- [ ] All ops writes go through the DuckLake writer via `ops_data_portal`; the portal caller surface (`file_rec`/`update_rec`/`file_decision`/`update_decision`/`sync`) is byte-for-byte unchanged (Decision 78 cl.6 / Decision 81 cl.4 -- Single-Portal invariant preserved, transport swapped).
...
| 6 | [pre-deploy] | Build writer + reader zips | `bin/venv-python -m scripts.build_lambda --ducklake-only` | writer + reader zips built + uploaded to S3 | Build error -> fix |
...
| 8 | [post-deploy] | Deploy writer + reader code | `bin/venv-python -m scripts.build_lambda --ducklake-only --deploy` | writer + reader code updated from S3 | Deploy error -> check function map |
| 9 | [post-deploy] | Create the ops_* DuckLake tables + run the backfill with parity verification | `bin/venv-python -m scripts.migrate_ops_iceberg_to_ducklake --execute --verify-parity` | Per-table `_current` row count + content match (excl. D70); `parity=PASS` | Parity FAIL -> STOP, RCA the mismatch; do NOT cut over (Decision 55) |
...
| 14 | [post-deploy] | Rollback rehearsal: flip to iceberg, confirm Iceberg path live, flip back | `OPS_STORAGE_BACKEND=iceberg bin/venv-python -m scripts.ops_data_portal --selftest-read && OPS_STORAGE_BACKEND=ducklake bin/venv-python -m scripts.ops_data_portal --selftest-read` | Both backends serve reads; Iceberg + ops_compaction confirmed intact | Rollback path broken -> fix BEFORE sign-off (rollback is the big-bang safety net) |
| 15 | [post-deploy] | CUTOVER SIGN-OFF: flip `OPS_STORAGE_BACKEND` default to `ducklake`; end-to-end portal write+read on DuckLake | `bin/venv-python -m scripts.ops_data_portal --selftest-roundtrip` (default backend) | A `file_rec`-shaped write lands in DuckLake and reads back via the portal; no Iceberg write occurs | Roundtrip fails -> revert the default flag; RCA |
...
2. **`src/common/ducklake_runtime.py`** -- table-parameterize `create_scd2_tables` / `write_scd2` / `read_current` / `schema_gate` / MERGE SQL (per-table, field_semantics-driven); keep smoke defaults for T2.17 back-compat; preserve mint-once-outside-OCC.
...
4. **`scripts/migrate_ops_iceberg_to_ducklake.py`** -- backfill + parity verification (excl. D70), idempotent, loud-fail on mismatch.
5. **`scripts/ops_data_portal.py`** -- transport swap behind `OPS_STORAGE_BACKEND` (writer Function URL), portal surface + ID alloc + outbox preserved; reads via the DuckLake reader.
...
7. **`config/agent/data_quality/ops.yaml`** + **`scripts/data_quality_runner.py`** -- re-point ops DQ backend to DuckLake; add clause-8 checks; preserve D64 anchor.
