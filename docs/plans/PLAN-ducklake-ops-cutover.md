...
| 4b | [pre-deploy] | Assert maintenance zip STAGES field_semantics.yaml (declaration added; `--check-bundles` won't catch a missing declaration) | `bin/venv-python -m scripts.build_lambda --ducklake-only && unzip -l dist/ducklake_maintenance.zip \| grep -q "config/lambda/ducklake/field_semantics.yaml" && echo STAGED` | `STAGED` printed | Not staged -> add the asset to `ducklake_maintenance/manifest.yaml` `assets[]` |
