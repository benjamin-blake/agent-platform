...
RECOMMENDATIONS portion of T2.19. **ROADMAP currently records T2.19 `status: not_started`; this plan's
roadmap edit transitions it `not_started` -> `in_progress`** (recs done), with decisions + `ops_session_log` /
...
| src/common/ducklake_runtime.py | Modify | Table-parameterize `create_scd2_tables` / `write_scd2` / `read_current` / `schema_gate` / MERGE SQL + `_SCD2_COLUMNS` + `SMOKE_*` constants (per-table, field_semantics-driven); **extend `_PY_TYPE_FOR_SQL`** to arrays/ints/booleans. **Parameterize `META_SCHEMA` in `open_connection()`** (currently hardcoded `ducklake_ops` in the ATTACH) -- add a `meta_schema` arg threaded through every writer/reader/maintenance/smoke caller, so production attaches `ducklake_ops` and smoke attaches `ducklake_smoke`. (Without this the smoke relocation -- the rec-2099 root-cause fix -- is not achievable.) Author + exercise `ops_recommendations` now; mechanism ready for later tables. Smoke back-compat retained (now under `ducklake_smoke`). Mint-once-outside-OCC preserved (CD.33 cl.2). |
...
| src/lambdas/ducklake_maintenance/manifest.yaml | Modify | Add `config/lambda/ducklake/field_semantics.yaml` to `assets[]` -- the seed/`catalog_reinit` actions call `schema_gate`+`write_scd2`, which `load_field_semantics()` at runtime; maintenance does not bundle it today (it has `assets: []`), and `compute_affected_artifacts` does NOT flag maintenance on a field_semantics change. Without this the seed loud-fails in-Lambda. (`--check-bundles` catches a declared-but-missing asset, NOT a missing declaration -- so the declaration must be added explicitly; VP4's `--check-bundles` then verifies it stages.) |
...
| docs/ROADMAP-PLATFORM.yaml | Modify | Transition T2.19 `not_started` -> `in_progress` (recs portion done); decisions + remaining ops tables tracked as the follow-up; **ops_compaction retirement DEFERRED** (stays live for non-migrated tables) -- T2.18 tail carries forward. Record rec-2099 resolved (reinit step added) + OQ.7/OQ.10 enacted for recs. |
...
| Transitively-affected artifacts (Decision 79) | `ops-compaction` + `data-pipeline` bundle `src/common/iceberg_reader.py` / `catalog_dr.py` via `includes: - src/`, so their zips change when those files do. The additions are ADDITIVE symbols (`DuckLakeReader`, the pg_restore helper) NOT imported by those handlers -> no behavioural delta. Run `compute_affected_artifacts(changed_files)` at implementation; rebuild them for byte-currency and run their existing smoke (no new behaviour to verify). Recorded here so the change is not silent against Decision 79. |
...
| 4 | [pre-deploy] | Validate writer/reader/maintenance manifests + bundles. Once the maintenance `field_semantics.yaml` asset is DECLARED (Fix 2), `--check-bundles` (`check_assets_present`) fails loud if it does not stage into the ducklake-maintenance bundle -- this is the canonical assertion that the seed has its schema source in-Lambda | `bin/venv-python -m scripts.lambda_manifest --validate && bin/venv-python -m scripts.lambda_manifest --check-bundles` | 3 functions validate; every declared asset stages, incl. `config/lambda/ducklake/field_semantics.yaml` in ducklake-maintenance | Asset declared but not staging -> fix the manifest `includes`; missing declaration -> add to `assets[]` |
...
- `ducklake_maintenance` is a singleton (`reserved_concurrent_executions=1`); the operational invokes (`catalog_reinit`/`seed`/`restore_drill`) can contend with the daily-merge / weekly-GC EventBridge schedule. Run them outside the scheduled windows, or temporarily disable the schedule for the cutover, to avoid throttle/queue.
- Cutover window relies on operator discipline: a `file_rec` issued by another session between seed (VP10) and flip (VP16) writes to Iceberg (still default); the VP16 outbox re-assert + incremental re-parity is the mitigation, but a sole-operator window is assumed.
...
2. **`ducklake_runtime.py`** -- table-parameterize the primitives + extend `_PY_TYPE_FOR_SQL` (arrays/ints/booleans); **add `meta_schema` to `open_connection()`** and thread it through all callers (production `ducklake_ops`, smoke `ducklake_smoke`); keep smoke back-compat.
...
4. **`ducklake_maintenance` handler + manifest** -- add `catalog_reinit`, `seed_ops_recommendations` (TEMP), `restore_drill` operational actions; add `config/lambda/ducklake/field_semantics.yaml` to the manifest `assets[]` (seed needs it in-Lambda).
