...
| 6 | [pre-deploy] | Build the ducklake zips (`--ducklake-only` builds all 4: writer, reader, maintenance, catalog-dr) | `bin/venv-python -m scripts.build_lambda --ducklake-only` | All 4 ducklake zips built + uploaded. Functional non-regression for maintenance/catalog-dr: their SOURCE is unchanged by this plan, so a rebuild is byte-equivalent behaviour (no code/config delta) -- this plan does not edit their handlers or flip their env | Build error -> fix |
...
| 8 | [post-deploy] | Deploy the ducklake function code (`--deploy` updates all 4) | `bin/venv-python -m scripts.build_lambda --ducklake-only --deploy` | writer + reader code live with the ops_* changes; maintenance/catalog-dr redeploy from unchanged source (no behavioural delta; their terraform-managed env is NOT flipped here) | Deploy error -> check function map; if maintenance smoke regresses, STOP (it should be byte-equivalent) |
| 9 | [post-deploy] | Drain + assert the Iceberg outbox EMPTY before backfill (so pending offline writes are captured), then create the ops_* DuckLake tables (re-create) + backfill + verify parity | `OPS_STORAGE_BACKEND=iceberg bin/venv-python -m scripts.ops_data_portal --sync && { find logs/.ops-outbox -type f \| grep -q . && echo "OUTBOX NOT EMPTY -- STOP" \|\| bin/venv-python -m scripts.migrate_ops_iceberg_to_ducklake --execute --verify-parity; }` | Outbox empty (all pending writes drained to Iceberg first); per-table `current` row count + content match (excl. D70); `parity=PASS` | Outbox not empty -> drain to Iceberg before backfill; Parity FAIL -> STOP, RCA (Decision 55), re-run re-creates tables (no append) |
