...
| 8 | pre-deploy | INTENT doc reconciled: no `ci-runner` row still marked BUILT without a retirement note, AND a CD.21 retirement note exists | `bin/venv-python -c "import sys; t=open('docs/INTENT-ci-cd-architecture.md').read(); stale=[l for l in t.splitlines() if 'ci-runner' in l and 'BUILT' in l and 'retir' not in l.lower()]; note=('CD.21' in t and 'retir' in t.lower()); print('OK' if not stale and note else 'FAIL stale=%d note=%s'%(len(stale),note)); sys.exit(0 if not stale and note else 1)"` | prints `OK` (exit 0) | prints `FAIL ...` -> the L8 row (~line 202) is still plain `BUILT` for `ci-runner`, or no `CD.21` retirement note was added; reconcile Sections 2.5/9 + lines ~230/~499 |
