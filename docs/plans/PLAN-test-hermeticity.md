...
| `pyproject.toml` | Modify | Add `--randomly-seed=last --disable-socket --allow-hosts=127.0.0.1,::1` to `[tool.pytest.ini_options]` addopts; register `network` marker |
...
| `tests/test_ducklake_spike.py` | Modify | Fix same module-level live-call pattern: defer to fixture |
