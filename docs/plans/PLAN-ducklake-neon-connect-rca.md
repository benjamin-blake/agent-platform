...
| src/lambdas/data-pipeline/manifest.yaml | Modify | Add `src/common/ducklake_connect_probe.py` to `excludes[]`. data-pipeline uses whole-tree `includes: - src/` and ALREADY excludes `ducklake_runtime.py` + `ducklake_scd2_schema.py` to keep DuckLake runtime code out of its minimal/closed-boundary zip; the new probe module is DuckLake runtime code the copytree would otherwise leak in. `compute_affected_artifacts` returns data-pipeline (whole-tree match), so this exclusion is REQUIRED for an honest Decision-79 affected-set. Mirrors the predecessor finalize plan's handling of `ducklake_scd2_schema.py`. |
| src/lambdas/ops-compaction/manifest.yaml | Modify | Same `excludes[]` addition (identical whole-tree + minimal-zip invariant as data-pipeline). |
...
| Lambda deployment (Decision 79 / CD.16) | `compute_affected_artifacts(changed_files)` over the real scope returns SIX active artifacts (confirmed live): the 4 DuckLake functions -- `ducklake_writer`, `ducklake_reader`, `ducklake_maintenance`, `ducklake_catalog_dr` -- PLUS the 2 transitively-matched whole-tree zips `data-pipeline` + `ops-compaction`. The 4 DuckLake functions bundle the byte-changed `ducklake_runtime.py` (+ writer/reader gain the new `connect_probe` action + the new module); ALL 4 are rebuilt + deployed for byte-currency via `build_lambda --ducklake-only --deploy`, with `ducklake_writer` + `ducklake_reader` the load-bearing smoke targets (connect_probe + lambda_attach). The 2 transitively-matched artifacts (`data-pipeline`, `ops-compaction`) EXCLUDE both `ducklake_runtime.py` (changed) AND the new `ducklake_connect_probe.py` from their minimal/closed-boundary zips -> their bundles are byte-UNCHANGED, so NO rebuild/deploy is required; the only action for them is ADDING the new module to their `excludes[]` and proving via `--check-bundles` (VP4) that they import-resolve WITHOUT it. This is the honest Decision-79 reconciliation: 6 affected, 4 deployed, 2 exclusion-only (byte-unchanged). No model IDs touched (deterministic SQL/sockets) -> inference-provider validation N/A. |
...
- [ ] Per-Lambda V3 (Decision 79): `compute_affected_artifacts` confirmed to return SIX artifacts. The 4 DuckLake functions rebuilt + deployed for byte-currency; writer+reader smoke-tested (connect_probe + lambda_attach); maintenance + catalog_dr existing smoke green (no behavioural delta beyond the timeout one-liner). The 2 transitively-matched zips (`data-pipeline`, `ops-compaction`) have `ducklake_connect_probe.py` added to their `excludes[]` and are confirmed byte-UNCHANGED (both `ducklake_runtime.py` and the new module excluded) -> exclusion-only, no rebuild required.
- [ ] Closed-boundary EXCLUSION proven: `--check-bundles` shows `data-pipeline` + `ops-compaction` import-resolve WITHOUT `ducklake_connect_probe.py` (it does not leak into the minimal/closed-boundary zips), mirroring the predecessor finalize plan's `ducklake_scd2_schema.py` exclusion.
...
| 4 | [pre-deploy] | Manifests validate; `--check-bundles` import-resolves the new module into the 4 DuckLake bundles AND confirms it is EXCLUDED from the 2 closed-boundary zips; confirm the affected-artifact set is six | `bin/venv-python -m scripts.lambda_manifest --validate && bin/venv-python -m scripts.lambda_manifest --check-bundles` then `bin/venv-python -c "from scripts.lambda_manifest import compute_affected_artifacts; print(sorted(compute_affected_artifacts(['src/common/ducklake_runtime.py','src/common/ducklake_connect_probe.py','src/lambdas/ducklake_writer/handler.py','src/lambdas/ducklake_reader/handler.py'])))"` | All active artifacts validate; `ducklake_connect_probe.py` import-resolves in writer/reader (+ maintenance/catalog_dr if they import it); `data-pipeline` + `ops-compaction` import-resolve WITHOUT it (excluded, byte-unchanged); affected set prints all SIX | A DuckLake bundle import-fails on the missing module -> add it to that manifest's `includes[]`; the module leaks into data-pipeline/ops-compaction -> add it to their `excludes[]` (the VP4 arbiter) |
...
- **Per-Lambda V3 (Decision 79 / CD.16).** Derive the affected set from `compute_affected_artifacts` (VP4),
  not by assertion -- it returns SIX. The 4 byte-affected DuckLake functions rebuilt + deployed (writer+reader
  smoke-tested); the 2 transitively-matched closed-boundary zips (`data-pipeline`, `ops-compaction`) are
  exclusion-only + byte-unchanged (the new module added to their `excludes[]`, proven via `--check-bundles`).
...
5. **Manifests** -- add `src/common/ducklake_connect_probe.py` to `includes[]` for writer + reader (and
   maintenance/catalog_dr iff `--check-bundles` flags a missing import at VP4); add it to `excludes[]` for
   `data-pipeline` + `ops-compaction` (whole-tree closed-boundary zips that already exclude
   `ducklake_runtime.py` -- stop the copytree leaking the new module in). VP4 confirms both directions.
