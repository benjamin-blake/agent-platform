...
| docs/INTENT-ci-cd-architecture.md | Modify | Affirm section 6 as the canonical platform-axis design. Replace the stale pre-CD.21 `company-aws-profile*` ACCOUNT-FAMILY tokens at the section 6 Environments table (lines 379, 380, 381 -- verify with `grep -n "company-aws-profile"`) with personal-account-family targets: sandbox = the current personal account; SIT/PROD = future dedicated accounts per Decision 76, gated on the live_full trigger. Cross-reference docs/contracts/environment-taxonomy.md. Reconcile any L9/L10 enforcement-surface rows that still imply company-account profiles. SEPARATELY: line 141 ALSO contains `company-aws-profile` but in a DIFFERENT semantic -- a Decision-57 `aws sso login --profile company-aws-profile` reference. Under the static-key model (PROJECT_CONTEXT line 99, NO SSO login) that line is stale REGARDLESS; fix it as an obsolete-SSO-login correction, NOT a sandbox/SIT/PROD account-family substitution. |
