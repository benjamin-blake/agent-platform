...
| docs/INTENT-ci-cd-architecture.md | Modify | Affirm section 6 as the canonical platform-axis design. Replace the stale pre-CD.21 `company-aws-profile*` ACCOUNT-FAMILY tokens at the section 6 Environments table (lines 379, 380, 381 -- verify with `grep -n "company-aws-profile"`) with personal-account-family targets: sandbox = the current personal account; SIT/PROD = future dedicated accounts per Decision 77, gated on the live_full trigger. Cross-reference docs/contracts/environment-taxonomy.md. Reconcile any L9/L10 enforcement-surface rows that still imply company-account profiles. SEPARATELY: line 141 ALSO contains `company-aws-profile` but in a DIFFERENT semantic -- a Decision-57 `aws sso login --profile company-aws-profile` reference. Under the static-key model (PROJECT_CONTEXT line 99, NO SSO login) that line is stale REGARDLESS; fix it as an obsolete-SSO-login correction, NOT a sandbox/SIT/PROD account-family substitution. |
...
| .github/workflows/ci.yml | Verify (no edit) | The `terraform-validate` job reads the pin DYNAMICALLY from `config/terraform-version` (verified lines 116-123: `setup-terraform@v4` with `terraform_version: ${{ steps.tfver.outputs.version }}`). There is NO hardcoded `1.5.0` here, so bumping `config/terraform-version` automatically flows to the CI terraform install -- no ci.yml edit needed. Listed for traceability; VP step 7's grep passes trivially. (Do NOT add a CI check without it existing in validate.py first.) |
| .github/workflows/deploy.yml | Modify | Update the hardcoded `terraform_version: 1.5.0` (the `setup-terraform@v4` input, ~line 40) to the new 1.10+ pin for consistency. NOTE: deploy.yml applies the WORK-account `terraform/` root (`cd terraform`), NOT `terraform/personal/`, so this bump is cosmetic consistency -- it does NOT enable `use_lockfile` (the work root stays `required_version >= 1.0`, retained-not-applied per CD.21). 1.10 satisfies `>= 1.0`, so the bump is harmless. |
...
| docs/PROJECT_CONTEXT.md | Modify | Re-word the "Terraform workflow integration" gotcha (line ~280): apply is human-gated EXCEPT sandbox auto-apply behind the deterministic guard, per Decision 77. |
| terraform/CLAUDE.md | Modify | Re-word the "Plan before apply" hard rule (line ~10) to the sandbox-scoped exception, pointing at Decision 77. |
| AGENTS.md | Modify | Re-word/qualify any unconditional "apply is never automatic" assertion to reference Decision 77's sandbox-scoped exception. |
...
- [ ] `docs/DECISIONS.md` contains "Decision 77", filed to the warehouse via the ops portal decision path (dec-NNN allocated atomically, not hand-written).
...
- [ ] No `1.5.0` TF pin remains in `config/terraform-version` or `.github/workflows/deploy.yml`; `config/terraform-version` is >= 1.10 (ci.yml reads it dynamically -- no hardcoded pin there).
...
- [ ] The "apply is never automatic" wording in `docs/PROJECT_CONTEXT.md`, `terraform/CLAUDE.md`, and `AGENTS.md` references Decision 77's sandbox-scoped exception.
...
| 3 | [pre-deploy] | Decision 77 present | `grep -q "Decision 77" docs/DECISIONS.md` | exit 0 | Author Decision 77 |
...
| 13 | [pre-deploy] | Doc rule references Decision 77 | `grep -q "Decision 77" docs/PROJECT_CONTEXT.md terraform/CLAUDE.md AGENTS.md` | exit 0 | Update wording |
...
- TF version pin MUST be bumped in the same change as `required_version >= 1.10`, or CI terraform-validate breaks on 1.5.0. The load-bearing edit is `config/terraform-version` (ci.yml reads it dynamically -- no ci.yml edit); deploy.yml's separate hardcoded pin is bumped for consistency only.
- No rescue agents or workaround loops (Decision 55). If a V3 step fails unrecoverably, stop and root-cause; do not paper over.
- Single Portal Invariant: Decision 77 is filed via `scripts.ops_data_portal` decision path; never hand-write the `dec-NNN` warehouse id or edit `.decisions-index.jsonl` directly.
...
- Guard detection contract (scripts/terraform_apply_guard.py), against `terraform show -json` output:
  ...
  - BLOCK if `.change.before` vs `.change.after` differs on an `assume_role_policy` / trust attribute on ANY resource (even otherwise-allowed types). NOTE: `terraform show -json` serialises `assume_role_policy` as a JSON-encoded STRING; normalise via `json.loads(...)` and compare the parsed structures, NOT raw string equality, so key-order/whitespace differences do not cause nuisance exit-2 trips. (Fail-closed makes a false positive safe -- it forces a manual admin apply -- but normalising reduces friction.)
...
3. Author Decision 77 in `docs/DECISIONS.md` (Decision 75 format: Status/Date/Warehouse ID/Problem/Decision); file it via the ops portal decision path so the warehouse id is allocated atomically.
...
8. Bump the Terraform version pin to >= 1.10: edit `config/terraform-version` (ci.yml reads it dynamically -- no ci.yml edit) and `.github/workflows/deploy.yml`'s hardcoded `setup-terraform` input (cosmetic; work-root only).
