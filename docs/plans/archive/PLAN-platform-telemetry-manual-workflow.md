...
| 9 | [post-deploy] | Compact outbox to Iceberg and query Athena | `AWS_PROFILE=company-aws-profile S3_LOG_BUCKET=agent-platform-agent-logs python -c "from scripts.ops_writer import OpsWriter; print(OpsWriter().compact_all())"` | Compaction reports telemetry_sessions records written | Fix OpsWriter config or S3 permissions |
