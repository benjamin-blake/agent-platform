...
| terraform/personal/ducklake_maintenance.tf | Create | Lambda function (from S3) + write-scoped exec role (S3 Get/Put/**Delete**/List on the smoke prefix, Neon DSN read, CloudWatch PutMetricData, logs) + 2 EventBridge schedule rules/targets/permissions (daily merge, weekly GC) + log group + `reserved_concurrency = 1` (singleton) + circuit-breaker CloudWatch metric alarm with NO action target (SNS page deferred to FP-B; no `aws_sns_topic` exists in `terraform/personal/`). |
...
| Item | Detail |
|------|--------|
| New resources | `aws_lambda_function.ducklake_maintenance`, `aws_iam_role.ducklake_maintenance` + inline policy, `aws_cloudwatch_log_group.ducklake_maintenance`, `aws_cloudwatch_event_rule` x2 (daily merge `cron(0 4 * * ? *)`, weekly GC `cron(0 5 ? * SUN *)`) + targets + `aws_lambda_permission` x2, `aws_lambda_function_url` (AWS_IAM, smoke-invoke ingress), `aws_cloudwatch_metric_alarm` (circuit-breaker; `alarm_actions = []` -- NO SNS target this slice), `reserved_concurrent_executions = 1`. **No SNS resource** -- `terraform/personal/` has none, and the page fan-out is deferred to FP-B (which creates one shared personal-account SNS topic for the breaker page AND the CD.34 catalog-DR `>25h` freshness alarm). |
...
- [ ] Circuit breaker aborts the destructive run when a single GC pass would delete > 20% of files OR > 10 GB: the abort raises (loud-fail, Decision 55), deletes nothing on that pass, and emits a CloudWatch metric + alarm. The alarm has NO action target in FP-A (`alarm_actions = []`); the SNS page fan-out is deferred to FP-B (no SNS topic exists in `terraform/personal/` -- FP-B creates one shared topic for this page and the CD.34 freshness alarm). The abort/delete-nothing half is the safety-critical invariant and IS fully in FP-A.
...
| 4 | [pre-deploy] | Terraform validate + plan the new resources | `terraform -chdir=terraform/personal validate && terraform -chdir=terraform/personal plan` | Plan adds the function, role, 2 schedules, log group, alarm (no SNS action target), reserved-concurrency=1; no new SNS resource; no destroys; present plan to human | Validate error (e.g. unresolved SNS ref) -> fix HCL; unexpected destroy -> stop (Decision 77 guard) |
...
| 11 | [post-deploy] | Live breaker gate: forced-threshold invoke trips the circuit breaker | `bin/venv-python -m scripts.ducklake_neon_smoke_test --lambda-maintenance-breaker` | Returns loud-fail (5xx / breaker error); deletes nothing; CloudWatch breaker metric emitted. (Do NOT assert on alarm-state transition -- it is timing-dependent and has no action target this slice; the metric + loud-fail + deletes-nothing are the load-bearing assertions.) | Breaker does not trip / deletes anyway -> fix the pre-deletion guard before destructive CALLs |
...
- **FP-B (OUT of scope, separate later plan):** (1) catalog DR (daily `pg_dump` of the Neon catalog -> versioned lifecycle S3 bucket, `cron(0 3 * * ? *)`, 30d retention, >25h freshness alarm -- CD.34 O-2); (2) telemetry small-file load co-tuning (higher-frequency merge for high-write-rate tables + circuit-breaker co-tune for the raised dead-file rate -- CD.34); (3) **a shared personal-account `aws_sns_topic`** wired as the action target for BOTH the FP-A circuit-breaker alarm AND the catalog-DR freshness alarm (the page fan-out deferred from FP-A -- human-confirmed). Because these T2.18 exit criteria are NOT met by FP-A, T2.18 stays open after this plan.
...
4. **`terraform/personal/ducklake_maintenance.tf`** -- function (from S3, layers reused), write-scoped exec role + inline policy (S3 Get/Put/Delete/List on smoke prefix, DSN read, PutMetricData scoped to a `DuckLakeMaintenance` namespace, logs), log group, 2 EventBridge rules + targets + lambda permissions, `reserved_concurrent_executions = 1`, Function URL (AWS_IAM), circuit-breaker `aws_cloudwatch_metric_alarm` with `alarm_actions = []` (SNS page deferred to FP-B -- no SNS topic in `terraform/personal/`), outputs for the smoke gates. Carry the reserved-concurrency NOTE comment and a `# FP-B: wire alarm_actions to the shared SNS topic` marker.
5. **`scripts/build_lambda.py`** -- extend `--ducklake-only` end to end: (a) add the third `build_ducklake_function_package(..., "ducklake_maintenance", "ducklake-maintenance.zip")` call in `_run_ducklake_build` (alongside the writer/reader build at ~lines 593-594), AND (b) add the function to `_DUCKLAKE_FUNCTION_ZIP_KEYS` / the `only_ducklake` deploy set + S3-key map. Both the build-package step and the deploy map must be extended -- not just the deploy map.
...
8. **`docs/runbooks/ducklake-catalog-operations.md`** -- add the maintenance-pipeline section (cadences, guardrail constants, breaker-alarm reading + the FP-A no-SNS-page caveat, manual-invoke runbook, reserved-concurrency NOTE, T2.19-expansion forward-pointer).
