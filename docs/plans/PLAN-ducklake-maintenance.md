...
| 11 | [post-deploy] | Live breaker gate: forced-threshold invoke trips the circuit breaker | `bin/venv-python -m scripts.ducklake_neon_smoke_test --lambda-maintenance-breaker` | Returns loud-fail (5xx / breaker error); deletes nothing; CloudWatch breaker metric emitted (alarm fires with no SNS action this slice) | Breaker does not trip / deletes anyway -> fix the pre-deletion guard before destructive CALLs |
