...
schema_version:
  ...
  semantics: |
    ...
    (forward-compat). The portal rejects schema_version > current_version
    (no silent fallback). Every breaking change to this schema increments the
    version and lands as its own follow-on plan with a documented migration
    for existing recs.
...
prior_art_citation:
  ...
  semantics: |
    ...
    The portal verifies each citation EXISTS (not just lexically valid):
      - decision-N looked up against ops_decisions_current
      - rec-N looked up against ops_recommendations_current
...
evidence_bundle_ref:
  ...
  fields:
    ...
    s3_uri:
      type: str
      required: true
      pattern: '^s3://bblake-platform-agent-logs/ci-rca-evidence/[0-9a-f]{64}\.json$'
    upload_status:
      type: enum
...
The full structured object is serialised as JSON and written to a NEW sibling column `context_v2_json` on `ops_recommendations` (added by PLAN-ci-rca-schema-enforcement via an Iceberg `ALTER TABLE ADD COLUMNS context_v2_json string`). The legacy `context` column receives a short human-readable summary (one paragraph, satisfies the existing `_validate_context_length` 80-character floor at `scripts/ops_data_portal.py:160`) so existing free-text consumers (`/plan` preflight render, audit dashboards, Athena `LIKE '%word%'` queries) keep working. This sibling-column approach is the migration-safe alternative to overloading the existing `context` field with JSON, which the adversarial critique flagged as a breaking change for current readers.

**Migration ordering.** The Iceberg schema migration (`ALTER TABLE ADD COLUMNS`) MUST complete and be observable from Athena BEFORE Phase 4 strict-mode flip (Section 6). PLAN-ci-rca-schema-enforcement enforces this ordering: the strict-mode flip is gated on a verification step that confirms the new column is present in the live table schema.
...
4. **Compute `earliest_viable_gate` recommendation.** Deterministic decision tree, applied in order:
   ...
   - If the timed probe fails or sample dispersion exceeds 50% of the median, set `earliest_viable_gate = null` and surface the failure in Section 3.4. The cross-check (Section 2) is skipped when the bundle value is null.
...
- **Taxonomy fallback (`unknown` category).** Function-name match fails AND log-pattern match fails. Bundle is emitted with `failure_category=unknown`, `classification_source="taxonomy_fallback"`, `earliest_viable_gate=null`. Agent files the main rec normally (with `recurrence_class=novel` and explicit acknowledgement of the unknown category in `gap_explanation`). Script ALSO emits a secondary rec via `source=ci_rca_taxonomy_extension` recommending taxonomy extension. The cross-check on `earliest_viable_gate` is skipped when bundle value is null.
- **AST parse failure on `validate.py`.** Bundle is emitted with `tier_membership=null`, `ast_walker_error=<parse error string>`, and `earliest_viable_gate=null`. Agent files the main rec but cross-check is skipped. Script emits a secondary rec via `source=ci_rca_static_analysis_failure` flagging the AST defect.
- **Timed runtime probe fails** (e.g. the check needs SSO and SSO is unavailable on the runner). Bundle reports `runtime_confidence="probe_failed: <error>"` and `earliest_viable_gate=null`. Decision-tree skips the runtime branch; the recommendation is based on dependency analysis only when dependency analysis is definitive (e.g. the check has external deps -> presubmit). Agent's `gap_explanation` field is still required.
- **Sample dispersion too high.** When dispersion exceeds 50% of median, `earliest_viable_gate=null` and the bundle records `runtime_confidence="dispersion_too_high: <details>"`. Agent files the main rec with `earliest_viable_gate` of their choosing (cross-check skipped); the dispersion is surfaced in Section 7 observability so the contract can detect systemic flakiness in the probe.
...
1. The `context_v2_json` argument MUST parse as the structured schema in Section 1 with `schema_version=1`. Schema versions greater than the current portal version are rejected. Missing the `context_v2_json` argument while `source=ci_rca` is a hard reject.
...
4. `detection_gap.actual_gate_that_caught_it` MUST equal `evidence_bundle.actual_gate_that_caught_it`. The bundle derives the value via the `workflow_to_tier` map (Section 3.2 step 2). For multi-failure runs producing N recs, all N share the same `actual_gate_that_caught_it`; the multiplexing is on `failed_check` (Section 4 check 9).
5. `recurrence_class in (instance_of_known_pattern, regression)` requires `prior_art_citation` to be present and non-empty. Each citation MUST match the patterns in the schema AND be verified to EXIST via lookup (Decision Records in `ops_decisions_current`, recs in `ops_recommendations_current`, doc paths via filesystem stat). Additionally, at least one citation MUST satisfy the relevance intersection rule in Section 1's `prior_art_citation.semantics`: appear in `evidence_bundle.related_recs_by_category` OR substring-match `failed_check` name OR match a Decision Record in `evidence_bundle.decision_records_cited`. Lexically-valid-but-non-existent citations are rejected with a specific diagnostic; existent-but-irrelevant citations (zero entries satisfying the intersection) are rejected with a diagnostic that lists the bundle-derived alternatives.
...
7. The `evidence_bundle_ref.sha256` MUST be present and pattern-valid. If `upload_status=ok`, the SHA-256 MUST exist as an S3 object at `evidence_bundle_ref.s3_uri`; missing object: reject. If `upload_status=upload_failed`, this check is the degraded path: accept the rec, do NOT verify S3 existence, queue the bundle for retry, surface the staleness in Section 7 observability. Recs with `upload_status=upload_failed` are flagged for /plan attention.
8. **`source=ci_rca_evidence_dispute` carve-out.** Recs filed against this contract's dispute path use their own schema (defined in PLAN-ci-rca-evidence-dispute) and are NOT subject to checks 1-7 above. They reference the parent ci_rca rec being disputed via a `parent_rec_id` field and follow the dispute schema's own validation. This carve-out is the only legitimate escape from the cross-check spine.
...
- **Phase 4 -- Back-validation, then flip to `strict`.** Once Section 7 observability shows the warn-mode rejection rate is consistently low (e.g. < 5% over 7 days of CI failures), run a back-validation step BEFORE flipping the flag: PLAN-ci-rca-schema-enforcement includes a CLI subcommand `bin/venv-python -m scripts.ops_data_portal back_validate --source ci_rca --since <phase-1-landing-date>` that re-validates every `source=ci_rca` rec written during the warn-period against the strict-mode schema. Each non-conformant rec is re-filed as a re-audit rec via `source=ci_rca_warn_period_audit` with the parent rec ID and a list of failing checks. This prevents adversarial behaviour entrenchment: any structurally-thin rec that shaped `/plan` architecture during the warn window is surfaced for human review before strict mode forecloses similar future filings. Only after back-validation completes AND its output is reviewed at `/plan` does the Decision Record approving the flip get authored. Then promote `CI_RCA_STRICT_MODE` to `strict` via the terraform path in Phase 0. Hard rejections fire.
...
The portal emits the following counters/gauges as structured log lines (consumed by the existing telemetry pipeline) and as rows in a new Athena table `ops_ci_rca_telemetry`:

- **Rejection counts by reason.** One row per rejected write: timestamp, agent run ID, rejection check number (1-9 from Section 4), rejection sub-reason (e.g. "field missing", "lexical match failed", "S3 object not found"), schema_version. Enables "are rejections trending up or down? which checks are biting hardest?" queries.
- **Override usage.** One row per `why_chain_terminus_override` use: parent rec ID, sibling override rec ID, reason summary. Enables detection of override-as-bypass behaviour.
- **Cross-check disagreement rate.** Bundle vs agent disagreement events with both values and the resolved-via-rejection outcome. Sustained high disagreement indicates either an agent prompt defect or a bundle script defect.
- **Bundle upload staleness.** Gauge: number of recs with `upload_status=upload_failed` whose bundle has not yet been retried successfully. Surfaces S3 outage impact.
- **Dispute path traffic.** Counter on `source=ci_rca_evidence_dispute` writes. Sustained traffic indicates a bundle script defect; near-zero traffic indicates agents are being silenced rather than disputing.
- **Recurrence-class distribution.** Per-week distribution of `recurrence_class` values across filed recs. Trend toward `novel` over time is the goal (recurring instances of known patterns should be designed out by the corresponding preventive_action plans).

#### 7.2 Preflight surfacing

`scripts/session_preflight.py` adds a new section `CI-RCA Telemetry (last 7d)` showing:
- Total ci_rca writes: N (accepted: M, rejected: K)
- Top 3 rejection reasons by count
- Override usage: X uses; sibling override recs: Y still open
- Bundle upload backlog: P pending
- Dispute path: Q open dispute recs

If rejections in `warn` mode exceed 25% of writes, or `strict`-mode rejections exceed 10%, the preflight surfaces a planning-context note: "ci_rca enforcement may need tuning -- consider a planning session on PLAN-ci-rca-strictness-tune".

**Threshold provenance.** The 25% / 10% / 5% (Phase-4 promotion gate) values are starting heuristics with no empirical grounding -- this contract is operationalising a new methodology, so the calibration curve does not yet exist. PLAN-ci-rca-observability-dashboard's first follow-on tuning rec is mandated to revisit these thresholds after a full week of Phase 4 data and either ratify them, adjust them, or replace the static thresholds with a rolling-baseline derivation (e.g. "alert if the 7-day rejection rate exceeds 2 standard deviations above the trailing 28-day mean"). Until that tuning rec lands, the three thresholds are read as "informational alerts to drive planning sessions", not as automated enforcement.

#### 7.3 Athena view

A view `ci_rca_health` joins `ops_recommendations_current` (filtered to `source=ci_rca`) with `ops_ci_rca_telemetry` to provide ad-hoc queries: "which validators are producing the most ci_rca recs?", "what's the average why_chain length?", "are recurrence_class=instance_of_known_pattern recs concentrated in a few files?".

## Follow-on plans

The following IMPLEMENTATION-type plans should be authored as separate `/plan` sessions to operationalise this methodology. Each is intended to be self-contained. All six plans roll up under `T1.12` in `docs/ROADMAP-PLATFORM.yaml` ("CI-RCA methodology contract -- deterministic depth via structured schema + evidence bundle"). T1.12 is a prerequisite of T3.4 (control-plane loop closure) -- the loop closure consumes well-formed RCA signals that this contract enforces. Soft coupling with T1.5 (ops_decisions graduation): if T1.5 lands first, PLAN-ci-rca-schema-enforcement migrates its implementation surface from `ops_data_portal.file_rec()` to the `log_rec` agent_sdk verb; not blocking but worth noting at plan-authoring time.
...
- **DECISIONS.md has two entries numbered Decision 72.** The current `docs/DECISIONS.md` shows `## Decision 72` at line 104 ("RCA-as-Plan-Source") AND at line 415 ("GitHub Branch Protection Not Available"). This contract cites the line-104 instance throughout. The duplicate is a doc-hygiene defect tracked separately (out of scope for this INTENT); cross-references in this document explicitly mean the line-104 instance.
