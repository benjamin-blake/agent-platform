...
**[STALE: Section 7 observability (ops_ci_rca_telemetry Athena table, ci_rca_health view) superseded by DuckLake closed boundary Decision 84/81; Section 7 implementation DEFERRED to PLAN-ci-rca-observability-dashboard.]** The portal emits the following counters/gauges as structured log lines (consumed by the existing telemetry pipeline) and as rows in a new table `ops_ci_rca_telemetry`:
...
#### 7.3 View **[STALE: Athena view superseded; deferred to PLAN-ci-rca-observability-dashboard with DuckLake reader surface]**
