...
| Roadmap detail (`files_in_scope`, `depends_on`) | `docs/ROADMAP-PLATFORM.yaml` | Typed-loader projection: `scripts.platform_roadmap.load()` (pure-local, no warehouse I/O -- distinct from the banned `-m scripts.platform_roadmap` module entrypoint) filtered to the ids already surfaced by the preflight cache; see the orient command Step 2 for the literal runnable form. Full-file Read only as an error fallback if the extraction fails. |
