...
- **Lambda deploy channel (Decision 125):** personal-account Lambda code deploys are decoupled from `terraform/personal` infra apply -- the governed CD path (see `docs/contracts/environment-taxonomy.md` section 5 and `docs/contracts/build-lambda.yaml`'s `deploy_channels`) is the default deploy channel; a local `bin/venv-python -m scripts.build_lambda --deploy` is break-glass only. Heuristic: when a production action (e.g. a Lambda code deploy) is auto-denied or has no obvious in-session path, grep `.github/workflows/` for a governed CD path before falling back to a local permission grant.
