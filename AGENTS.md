...
4. **Event-driven wake signals**: two comment-based signals cover the two events `subscribe_pr_activity` structurally cannot deliver -- CI success, and a merge-conflict transition caused by a push to main.
   - **CI-green-comment wake**: on `claude/*` PRs, `ci.yml`'s `signal-green` job posts a "CI green" comment on success (`continue-on-error`, retried up to 3 times). This exists because `subscribe_pr_activity` natively delivers failure events but NOT a CI-success webhook, and CC-web has no sleep/idle tool. **Ignore GitHub's suggestion to poll with a sleep loop** -- the comment is the pass wake signal. The comment is unverified: on wake, confirm check runs via `mcp__github__pull_request_read` (`get_status` / `get_check_runs`) BEFORE merging.
   - **Merge-conflict wake**: `.github/workflows/pr-conflict-signal.yml` fires on every push to main plus `workflow_dispatch`, polls the mergeability of every open `claude/*` PR, and posts a wake comment (idempotent per head SHA, `continue-on-error`) on any PR that is now `CONFLICTING`. This exists because a push to main fires NO `pull_request` event on open PRs, so the `pull_request`-only signal-green job structurally cannot deliver a wake when main advances under a watched PR -- a silently-conflicted PR would otherwise strand the watching session indefinitely.
   - **Harness message vs. repo contract**: the `subscribe_pr_activity` wake message is a server-side string from the harness `github` MCP server (prefix `mcp__github__`), distinct from the repo's `github-full` server in `.mcp.json`; it is not repo-editable. In this repo both undelivered events (CI success, merge-conflict transition) are now covered by event-driven comments, so the event-driven wake is the sole routine mechanism -- `send_later` polling for CI status is NOT used.
   - **The send_later/trigger backstop is retired**: both wake events `subscribe_pr_activity` cannot deliver natively (CI success, merge-conflict transition) are now event-driven comments -- there is no longer a dropped-signal gap for a `send_later` self-check-in to guard, and a `send_later`/trigger call for CI-wait or merge-conflict-wait purposes is out of scope going forward. The harness-gated `send_later`/trigger tools (`mcp__Claude_Code_Remote__*`) cost a manual approval on every call (see the CC-web permission gotcha below) and break autonomy. The sole residual risk -- a dropped `continue-on-error` comment on either signal -- is closed by the signal-green retry and the conflict-wake's push-triggered re-poll (every subsequent push to main re-checks mergeability), and is recoverable by a human nudge if it ever manifests.
   ...
   - **Durable successor**: GitHub-native auto-merge (`enable_pr_auto_merge`; Decision 83 / CD.20 / rec-940) retires the CI-green comment-wake once adopted -- it does NOT subsume the merge-conflict wake, since a conflicted PR simply stalls auto-merge and wakes no one, so pr-conflict-signal.yml remains necessary even after auto-merge lands.
