...
locals {
  ...
  # T2.19 / T2.26 (c4): all three migrated-table _current views (ops_recommendations_current,
  # ops_decisions_current, ops_priority_queue_current) are DROPPED (Decision 81 cl.7; Decision 84).
  # All recs/decisions/queue reads transit the DuckLake reader closed boundary; no compliant
  # consumer reads these views post-cutover. See null_resource.drop_ops_recommendations_view,
  # null_resource.drop_ops_decisions_view, and null_resource.drop_ops_priority_queue_view below
  # for the explicit DROPs.
...
# ---------------------------------------------------------------------------
# T2.26 (c4): explicitly DROP ops_decisions_current (Decision 84 cutover).
#
# Why a one-shot resource: removing the ops_decisions_current key from the for_each map above
# destroys the null_resource in state but issues NO DDL -- the live Glue view persists. An
# explicit DROP VIEW is required. This resource runs ONCE (trigger tied to a constant) so
# re-apply does not re-issue DDL after the view is already absent.
#
# APPLY POSTURE: null_resource delete is a destroy event. The terraform_apply_guard.py
# fail-closes on any destroy/IAM/trust change (Decision 77). This apply routes to
# tf-gated-apply Environment approval.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# T2.26 (c4): explicitly DROP ops_priority_queue_current (Decision 84 cutover).
#
# Why a one-shot resource: removing the ops_priority_queue_current key from the for_each map
# above destroys the null_resource in state but issues NO DDL -- the live Glue view persists. An
# explicit DROP VIEW is required. This resource runs ONCE (trigger tied to a constant) so
# re-apply does not re-issue DDL after the view is already absent.
#
# APPLY POSTURE: null_resource delete is a destroy event. The terraform_apply_guard.py
