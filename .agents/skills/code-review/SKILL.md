...
**The caller MUST use `python scripts/ops_data_portal.py` to file each finding. Never append to `logs/.recommendations-log.jsonl` directly.** This ensures ID authority via DynamoDB and persistence via the OpsWriter.
